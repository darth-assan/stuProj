...
def merge_seperately(df):
    # Separate rows into tables based on attack columns
    df_attack = df[df['attack'] == 1]
    df_attack_P1 = df[df['attack_P1'] == 1]
    df_attack_P2 = df[df['attack_P2'] == 1]
    df_attack_P3 = df[df['attack_P3'] == 1]
    df_none = df[
        (df['attack'] != 1) &
        (df['attack_P1'] != 1) &
        (df['attack_P2'] != 1) &
    ...
    print(f"df_attack shape: {df_attack.shape}")
    print(f"df_attack_P1 shape: {df_attack_P1.shape}")
    print(f"df_attack_P2 shape: {df_attack_P2.shape}")
    print(f"df_attack_P3 shape: {df_attack_P3.shape}")
    print(f"df_none shape: {df_none.shape}\n\n\n")

    return df_attack, df_attack_P1, df_attack_P2, df_attack_P3 
...
print("Testing HAI:")
dftest_attack, dftest_attack_P1, dftest_attack_P2, dftest_attack_P3 = merge_seperately(dftest_2103)
print("Training HAI:")
dftrain_attack, dftrain_attack_P1, dftrain_attack_P2, dftrain_attack_P3 = merge_seperately(dftrain_2103)

#pd.set_option('display.max_rows', dftest_attack_P1.shape[0]+1) # some attacks are in union
#print(dftest_attack_P2)
